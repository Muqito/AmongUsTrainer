...
fn main() {
    ...
    let module = hack_util::Module::get_module("Among Us.exe", "Among Us.exe");

    //let player_control_ptr = process.pointer_from_offsets(0, vec![0x785A0000, 0x5C, 0x0]);
    //let game_options_data_ptr = process.pointer_from_offsets(0, vec![0x785A0000, 0x5C, 0x4]);
    ...
    loop {
        ...
        let speed_ptr = process.pointer_from_offsets(0x785A0000, vec![0xDA3C30, 0x5C, 0x14]) + 0x14;
        if speed {
            ogspeed = process.read_memory::<f32>(speed_ptr);
            process.write_memory::<f32>(speed_ptr, 3.0);
        } else {
        ...
        if cooldown {
            let kill_cooldown_ptr =
                process.pointer_from_offsets(0x785A0000, vec![0xDA5A84, 0x5C, 0x0]) + 0x44;
            process.write_memory::<f32>(kill_cooldown_ptr, 0.0);
        }

        let kill_range_ptr =
            process.pointer_from_offsets(0x785A0000, vec![0xDA5A84, 0x5C, 0x4]) + 0x40;
        if range {
        ...
        if vent {
            let vent_move_ptr =
                process.pointer_from_offsets(0x785A0000, vec![0xDA5A84, 0x5C, 0x0]) + 0x30;
            let in_vent_ptr =
                process.pointer_from_offsets(0x785A0000, vec![0xDA5A84, 0x5C, 0x0]) + 0x31;
        ...
        let player_imposter_ptr =
            process.pointer_from_offsets(0x785A0000, vec![0xDA5A84, 0x5C, 0x0, 0x34]) + 0x28;
        ...
        let vision_radius_ptr =
            process.pointer_from_offsets(0x785A0000, vec![0xDA5A84, 0x5C, 0x0, 0x54]) + 0x1C;
